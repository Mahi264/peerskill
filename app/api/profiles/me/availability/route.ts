...
import { updateAvailabilitySchema } from "@/lib/validations/profile";
...
export async function PATCH(request: Request) {
    ...
    return NextResponse.json(
      {
        error: {
          ...
          message: "Profile must be created before updating availability settings.",
        },
      },
      { status: 422 },
    );
  ...
  const { helpAvailable, helpStatus } = result.data;

  const profile = await prisma.profile.update({
    where: { userId: user.id },
    data: {
      ...(helpAvailable !== undefined && { helpAvailable }),
      ...(helpStatus !== undefined && { helpStatus }),
    },
  });
